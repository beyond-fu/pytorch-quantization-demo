...
# for PTQ, calculate min, max, quantize, dequantize
...
# basic class of basic nn block to be quantized
class QModule(nn.Module):
    # not all nn block has qi and qo
    ...
    # pre-calculate and quantize some params
    def freeze(self):
        pass
...
class QConv2d(QModule):
    ...
    # inference on the float, introduce fake quantization error
    def forward(self, x):
        if hasattr(self, "qi"):
            self.qi.update(x)
        ...
        x = F.conv2d(
            x,
            FakeQuantize.apply(
                self.conv_module.weight, self.qw
            ),  # fakequant for weight
